...
// account for one small cave visited twice, every other small cave once
...
function dupeExists(list) {
    let counter = {};
    list.forEach(item => {
        if (item.toLowerCase() != item) return;
...
function determinePaths(path) {
    ...
    next.forEach(set => {
        ...
        if (dupeExists(copy) && copy.includes(set) && set.toLowerCase() == set) return;
        copy.push(set);
