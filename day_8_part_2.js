...
const positions = [ // index: segment positions required to build it
    [0, 1, 2, 3, 4, 5],
    [1, 2],
    [0, 1, 3, 4, 6],
    [0, 1, 2, 3, 6],
...
// determine how many times each segment position occurs
const occurrences = [];
for (let i = 0; i < 7; i++) {
    let count = 0;
    positions.forEach(pos => {
        count += pos.filter(num => num == i).length;
    });
    occurrences.push(count);
}

input.forEach(row => {
    const known = {}; // letter: position
    const unique = {}; // num: segmentString
    const found = [];

    // painfully solving for each letter
    row[0].forEach(element => {

        // using occurrences to discover what segment position letters correlate to
        let letterOccurs = {};
        row[0].forEach(segment => segment.split("").forEach(letter => {
            if (!letterOccurs[letter]) letterOccurs[letter] = 0;
            letterOccurs[letter]++;
        }));
        occurrences.forEach((occurrence, index) => {
            if (occurrences.filter(element => element == occurrence).length != 1) return;
            const letter = Object.keys(letterOccurs).find(letter => letterOccurs[letter] == occurrence);
            known[letter] = index;
        });

        // finds case where a number is simply a copy of the last with an extra number, uses to find extra number
        while (Object.keys(known).length < 7) {
            row[0].forEach(segment => {
                // strip from all "known"
                let segmentCopy = segment;
                Object.keys(known).forEach(letter => segmentCopy = segmentCopy.replace(letter, ""));

                // if only one, add to known
                if (segmentCopy.length != 1) return;
                let len = segment.length;

                // use letterOccur to find how often the letter occurs
                let occurs = Object.keys(letterOccurs).filter(letter => letterOccurs[letter] == letterOccurs[segmentCopy[0]]);
                if (occurs.length == 2) {
                    let set = positions.filter(pos => pos.length == segment.length);
                    set.forEach((pos, index) => {
                        Object.values(known).forEach(num => set[index] = set[index].filter(element => element != num));
                    });
                    set.forEach(pair => {
                        if (pair.length == 1) known[segmentCopy[0]] = pair[0];
                    })
                } else {
                    known[segmentCopy[0]] = occurrences.find(occurs => occurs == letterOccurs[segmentCopy[0]]);
                }
            });
        }
    });
    
    // build every unique based on positions
    positions.forEach((set, index) => {
        let str = "";
        set.forEach(num => {
            str = str.concat(Object.keys(known).find(letter => known[letter] == num));
        });
        found.push(str.split("").sort().join(""));
    });

    // builds end numbers
    let num = "";
    row[1].forEach(segment => {
        num = num.concat(found.indexOf(segment.split("").sort().join("")));
    });
    res += parseInt(num);
});
console.log(res);